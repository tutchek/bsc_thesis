...
There can be also additional condition on the quasigroup member values which makes
the problem more difficult. List of the possible conditions can be found at \cite[prob003]{csplib}.
We will use condition QG3.m which states that

$$(ab)(ba) = a$$

In this case multiplication is not standard multiplication on integers. Value of
$ab$ is value in the table in the row $a$ and column $b$.

\begin{figure}
\caption{Quasigroup Completion Problem written in Essence'}
\begin{verbatim}
...
such that
  ...
  forall i : nDomain. forall j : nDomain. 
	( i = qcp[ qcp[i,j] , qcp[j,i] ] )
\end{verbatim}
\end{figure}
