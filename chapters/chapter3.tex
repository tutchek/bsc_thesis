...
Listed benchmarks allows us test listed constraint solvers in various aspects. Model
for self refferential quiz stands on reification. It shows how the system can handle 
with reification both on boolean and discrete domain variables. N queens and magic 
sequences are scalable benchmarks which can be used to test the robustness of solvers
as well as how it behave if the problem grows. Finally quasigroups problems shows
how the solver can handle with large "close to world" random problem. 

\section{Self Referential Quiz}
\zkratka{SRQ}{Self Referential Quiz} was used in \cite{fernandez00}. More general 
description of the puzzle and how to solve it and even how to generate new self referential
quizes can be found in \cite{jios:bubalo}. The quiz consist from ten questions whose 
answers depends one on another.
...
quasigroup. The table is filled with numbers which are consistent 
with the latin square condition. By different initial filling we bring into model perturbances which can
more or less change behaviour of the solving process. However the problem can still result as unsolvable.
...
number 3 is. Probability of unsolvable CSP grows with percentage of original filling.
...
table $p$ and size of table $N$. 

This benchmark allows to perform test closer to real world applications becauce it
has structure which is not found in random k-SAT and by scaling of the parameter $p$
we can controll hardness of benchmark. However as stated before, we does not have guarantee
that problem is solvable. So there exists assignments with same $p$ parameters but
different search progress -- if solver did not find solution we are unsure if it is
because of hardness of problem (and solver limitation) or because there is no solution.

Better would be benchmark with properties of QCP but allways solvable, so there
will not be different ways to achieve results at one $p$ level. The answer is Quasigroup
With Holes described in following section.  
...
QHW has evenly distributed non-holes across rows and columns. In this benchmark 
we will generate several quasigroups and then "punch" holes to them.

As stated in \cite{Achlioptas00generatingsatisfiable} QWH is a problem generator 
for satisfiable instances (only):

\begin{itemize}
\item Easy to tune hardness
\item Exhibits more realistic structure 
\item Well-suited for the study of incomplete search methods (as well as complete)
\item Confirmation of easy-hard-easy pattern in computational cost for local search
\end{itemize}

The result problem of both Quasigroup problems is the same - to fill the latin 
square but only QWH warrants that the problem is solvable.

Because of similarity of problems QCP and QWH we will not perform separate benchmarking
for both of them. Instead we will perform benchmarks on quasigroups testing performance 
of finding solution of QWH with different percentage of filling and performance of 
finding that there is not solution on given unsolvable quasigroups.
