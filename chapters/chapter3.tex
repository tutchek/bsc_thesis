...
který tento jazyk nikdy nevidìl, byl schopen urèit, co daný program dìlá. Pro pøeklad 
programù v Essence do jiných jazykù existuje volnì dostupný pøekladaè Taylor. 
Tento pøekladaè umí pøelo¾it zadaný program na zdrojový kód pro øe¹iè Gecode 
pøípadnì na vstupní soubor pro øe¹iè Minion. Samotný program v Essence sestává 
ze tøí èástí. První èást definuje verzi jazyka, druhá èást pou¾ité promìnné a 
koneènì tøetí omezující podmínky na tìchto promìnných. Pou¾ité promìnné mohou být 
typu Integer, Boolean a vektor/matice. 
...
\subsection{Implementace v Essence}
...
\begin{figure}[ht]
\caption{Implementation of N-Queens Problem in Essence}
\begin{lstlisting}
...
\begin{itemize}
	...
	\item Podmínky:
	 \begin{itemize}
      ...
      {0,...,k-1}: m_i = \sum{m_j = 1}{1}$.
    \end{itemize}
\end{itemize}
  
V pøípadì, ¾e nelze v øe¹ièi pou¾ít podmínku $m_i = \sum{m_j = 1}{1}$, dá se sestavit 
alternativní model
  ...
  \begin{itemize} 
    ...
    \item $\mathrm{aux}_ij$ má hodnotu 1 pokud $m_j$ má hodnotu i, jinak má 
	  ...
	  \item Prvky magické sekvence odpovídají souètu pøíslu¹ných pomocných promìnných: 
          $\forall i \in \left\{0, ..., k-1\right\}: m_i = \sum_{j=0}^{k-1}{\mathrm{aux}_ij}$.
    \end{itemize}
\end{itemize}
...
"1. První otázkou na kterou je odpovìï A je: A: 1, B: 2, C: 3, D: 4, E: na ¾ádnou 
otázku není odpovìï A" a "2. Odpovìdí na tuto otázku je: A: A, B: B, C: C, D: D, E: E". 
...
\subsection{Model CSP}
	\begin{itemize}
  \item Promìnné a domény: $s_{1|A}, s_{1|B}, ..., s_{10|D}, s_{10|E} \in {0, 1}$.
	\item Podmínky:
	 \begin{itemize}
    \item V ka¾dém øádku je právì jedna hodnota rovna 1: $\forall i \in {1, ..., 10}: \left(\sum_{j \in \{A, ..., E\}}{s_{i|j}}\right) = 1$.
   \end{itemize}
	\end{itemize}
...
\subsection{Model CSP}
  ...
  \item Promìnné a domény: 
	\item Podmínky:
  	\begin{itemize}
  	 \item Objektivní funkce: $\mathrm{totalCost} = \sum_{w \textnormal{ is open}}\left(c+\sum_{s \textnormal{ is suplied by } w} \mathrm{SC}(s,w) \right)$
    \end{itemize}
  \end{itemize} 