...
The constraint programming is a programming paradigm which uses constraints to 
...
solved using the constraint programming we use the Sudoku puzzle. Sudoku is a worldwide 
...
straightforward. We define the following constraints:
...
\item	For all prefilled values: $q_{mn} = V$ if and only if the field with placed in column 
...
then you know the constraint programming." The formal definitions of the constraint satisfaction
...
just like the person solving Sudoku described in the previous paragraphs.
...
A programmer who wants to solve problems using constraint programming can use in their 
algorithms ideas described in the previous paragraphs or use specialized software, a 
...
which covers a similar area; however, this paper does not focus on the user experience 
...
capabilites of these solvers and showed ways to debug the models. Finally the speed 
of solvers was measured on several benchmarks. In this thesis we study wider range of
